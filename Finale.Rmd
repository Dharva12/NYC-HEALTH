...
```{r}
# Dropping columns with more tha 5% missing values
...
lr_model <- cv.glmnet(as.matrix(x_train), as.factor(y_train), alpha = 0.5, family = "multinomial")

# Use the lambda values selected by cv.glmnet
lr_best_lambda <- lr_model$lambda

# Train the final model with the best hyperparameters
lr_final_model <- cv.glmnet(as.matrix(x_train), as.factor(y_train), alpha = 0.5, lambda = lr_best_lambda, family = "multinomial")

# Make predictions on test data
lr_predictions_test <- predict(lr_final_model, newx = as.matrix(x_test), type = "class")

lr_predictions_test <- factor(lr_predictions_test)

# Calculate accuracy on test data
lr_accuracy_test <- sum(lr_predictions_test == y_test) / length(y_test)

lr_conf_matrix <- confusionMatrix(lr_predictions_test, y_test)

levels(y_test)

print(lr_conf_matrix)

print(length(lr_predictions_test))
print(length(y_test))

# Print the accuracy on test data
print(paste("Test Accuracy:", lr_accuracy_test)) # "Test Accuracy: 0.652248150256118" 
...
# levels(train_data$k6categorical)
...
# # Print the accuracy on test data
print(paste("Test Accuracy:", lr_2_accuracy_test)) # "Test Accuracy: 0.657939669891861" BETTER ACC 
...
print(paste("Accuracy:", svm_best_accuracy)) # 0.642572566875356
...
knn_predicted_labels <- best_knn_model # Best Accuracy: 0.5788275 
...
# print(index)
# 
# # Specify hyperparameter values to tune
# mtry_values <- c(2, 4, 6, 8, 10, 12, 14)  
# ntree_values <- c(10, 200, 300, 400, 500, 600) 
...
# rf_best_model <- NULL
# 
# nyc_filtered$k6categorical <- as.factor((nyc_filtered$k6categorical))
# levels(nyc_filtered$k6categorical)
# 
# #levels(y_test)
# 
...
#     for (maxdepth_value in maxdepth_values) {
#       
#       # Train Random Forest model
#       rf <- randomForest(k6categorical ~ ., data = nyc_filtered,
#                          subset = index, mtry = mtry_value,
#                          ntree = ntree_value, max_depth = maxdepth_value,
#                          importance = TRUE)
#       
#       # Make predictions
#       rf_predict <- predict(rf, x_test)
#       
#       # Calculate accuracy
#       rf_conf_matrix <- table(rf_predict, y_test)
#       rf_accuracy <- sum(diag(rf_conf_matrix)) / sum(rf_conf_matrix)
#       
#       rf_accuracy <- sum(rf_predict == y_test) / length(y_test)
#       
#       # Print accuracy for the current hyperparameters
#       cat("mtry:", mtry_value, "ntree:", ntree_value, "max_depth:", maxdepth_value, "Accuracy:", rf_accuracy, "\n")
#       
#       # Update the best model if the current model has higher accuracy
#       if (rf_accuracy > rf_best_accuracy) {
#         lr_best_accuracy <- rf_accuracy
#         lr_best_model <- rf
#       }
#     }
#   }
...
# cat("Best Accuracy:", rf_best_accuracy, "\n")
# cat("Best Model:", "\n")
# print(rf_best_model)
# 
...
for (maxD in maxdepth_values ){
  ...
  return(list(best_model =  rf_best_model, best_accuracy = rf_best_accuracy, 
              best_ntree = rf_best_ntree, best_mtry = rf_best_mtry, best_maxDepth =  rf_best_maxD))
}
...
#plot(rf_result$best_model)

feature_importance <- importance(rf_result$best_model)

print(feature_importance)

# Plot feature importance
ggplot(data = feature_importance, aes(x = row.names(feature_importance), y = MeanDecreaseAccuracy)) +
